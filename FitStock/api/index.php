    ...
    switch ($resource) {
        ...
        case 'prestamos':
            ...
            if ($method === 'GET') {                         // GET /api/prestamos - Listar todos con JOINs
                ...
                jsonResponse(["error" => "Datos inválidos"], 400);
            } elseif ($method === 'PUT' && isset($path[2])) {   // PUT /api/prestamos/{id} - Marcar como devuelto
        ...
        case 'productos':
            ...
            if ($method === 'GET') {                         // GET /api/productos - Listar todos
                ...
                if ($nombre) {
                    Producto::crear($nombre, $cantidad, $stock_minimo, $precio);
                    jsonResponse(["success" => true]);
                }
                jsonResponse(["error" => "Datos inválidos"], 400);
            } elseif ($method === 'PUT' && isset($path[2])) {   // PUT /api/productos/{id} - Actualizar stock
                $cantidad = intval($data['cantidad'] ?? -1);
                if ($cantidad >= 0) {
                    Producto::actualizarStock($path[2], $cantidad);
                    jsonResponse(["success" => true]);
                }
                jsonResponse(["error" => "Cantidad inválida"], 400);
        ...
        case 'incidencias':
            ...
            if ($method === 'GET') {                         // GET /api/incidencias - Listar todas
                ...
                // Si la incidencia se resuelve, la maquina vuelve a operativo
                if ($estado === 'resuelta') {
                ...
                $inc = Incidencia::obtenerPorId($path[2]);
                $idMaterial = $inc ? $inc->getIdMaterial() : null;
                Incidencia::eliminar($path[2]);
                // Si no quedan incidencias abiertas para ese material, vuelve a operativo
                if ($idMaterial) {
                    $conexion = Conexion::conectar();
                    $stmt = $conexion->prepare("SELECT COUNT(*) FROM incidencias WHERE id_material = ? AND estado_inc != 'resuelta'");
